(function() {
	$('form').ajaxForm({
		beforeSubmit: function() {	
			...
			if( val == '' ){
				count= 1;
				$( "#images" ).next('span').html( "Please select your File" );
			}
			...
			if(count == 0){
				for (var i = 0; i < $('#images').get(0).files.length; ++i) {
			    	...
			    	if(extension!="PNG" && extension!="JPG" && extension!="GIF" && extension!="JPEG" && extension!="PDF" && extension!="DOC" && extension!="DOCX" && extension!="XLS" && extension!="XLSX" && extension!="PPT" && extension!="PPTX"){
			    		count= count+ 1
			    	}
			    }
				if( count> 0) $( "#images" ).next('span').html( "Please select valid File" );
			}
